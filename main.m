close all;
seed = 0; 
my_home = '/Users/abhinav/Desktop/Submod-code/non-monotone-dr-submodular/code/';
%rng(seed,'twister');
rand();
%addpath('/Users/abhinav/Desktop/Submod-code/non-monotone-dr-submodular/src/');
addpath(strcat(my_home,'src'));
%
%
% add path of CPLEX, e.g., on my machine, it is
% '/Applications/CPLEX_Studio128/cplex/matlab/x86-64_osx'
...
%
%% setup
% different way of genering data
%
%
data_id = 4
%
...
%
%
solver_names = {'our-Frank-Wolfe', 'quadprogIP','our-proj-gradient','non-monotone Frank-Wolfe variant', 'two-phase Frank-Wolfe'};
%
nm_names=length(solver_names);
result_path = strcat(my_home,'results/');
...
if 1 == data_id | 2 == data_id % for quadratic functions, use global-optimal solver quadprogIP
    solver_list = [1 2 3 4 5]   %for down-closed polytope
else
    solver_list = [1 3]         %for general polytope (Make changes in src/gen_data.m file)
end
%
%
...
%
K = 5;
% number of trials for different dimensionalities
%
%
nbase = 8;
n_stepsize = 2;
...
for mn_type = 1:3
    ...
    plot_opt = {'--db', ':^r', '--sm',...
        '-.vk','--*g','-.xc'};  % line config. for the solvers
    close all
