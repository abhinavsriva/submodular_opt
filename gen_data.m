...
switch dataset
    case 1, %  quadratic data, uniform distribution
        ...
        for j = 1:n
            ub(j) = min(b./A(:,j)); %+ (0.25)*(max(b./A(:,j)) - min(b./A(:,j)));
        end

        %% Uncomment these line for general polytope
        %b(m-1) = -1;
        %for j = 1:n
        %  A(m-1,j) = -1;
        % end
        ...
        [y, f_min, ~, ~] = quadprogIP(H,h, A,b,param.Aeq,param.beq,lb,ub, param.opts_quadprogIP);
